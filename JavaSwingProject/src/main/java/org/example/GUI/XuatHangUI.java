...
public class XuatHangUI extends JPanel implements ActionListener{
      ...
      public List<SanPham_DTO> getDanhSachSanPhamTaoPhieuXuatHang() {
            XuatHang_DAO xuatHang_DAO = new XuatHang_DAO();
            List<SanPham_DTO> danhSachSanPhamTaoPhieuXuatHang = new ArrayList<>();
            for(int i = 0; i < table_ds_xuat_hang.getRowCount(); i++) {
                  int maSanPham = (Integer)table_ds_xuat_hang.getValueAt(i, 1);
                  SanPham_DTO temp = xuatHang_DAO.getSanPhamTheoMaSanPham(maSanPham);
                  temp.setSoLuongXuatHang((Integer)table_ds_xuat_hang.getValueAt(i, 3));
      ...
      public void taoPhieuXuatHang() {
            ...
            try {
                  ...
                  //? UPDATE CTPXK
                  for (SanPham_DTO sanPham : getDanhSachSanPhamTaoPhieuXuatHang()) {
